...
function feat(y: number, t: string, d: string, ic: "target" | "scale" | "dollar"): string {
  const icons: Record<string, string> = {
    ...
    dollar: `<circle cx="62" cy="${y + 22}" r="13" fill="none" stroke="#22E06B" stroke-width="2"/><text x="62" y="${y + 27}" text-anchor="middle" font-family="Arial" font-size="15" font-weight="700" fill="#22E06B">$</text>`,
  };
  return `<rect x="40" y="${y}" width="44" height="44" rx="10" fill="#0F1A12" stroke="#22E06B" stroke-width="1.5"/>${icons[ic]}`
    + `<text x="98" y="${y + 18}" font-family="Arial" font-size="15" font-weight="700" fill="#FFFFFF">${t}</text>`
    + `<text x="98" y="${y + 37}" font-family="Arial" font-size="12" fill="#938ea1">${d}</text>`;
}
...
function bar(y: number, nome: string, preco: string, w: number, bc: string, tc: string, bold: boolean): string {
  ...
  return `<text x="60" y="${y + 5}" font-family="Arial" font-size="13" ${b}fill="${tc}">${nome}</text>`
    + `<rect x="150" y="${y - 4}" width="${Math.max(24, w)}" height="14" rx="7" fill="${bc}"/>`
    + `<text x="620" y="${y + 5}" text-anchor="end" font-family="Arial" font-size="13" ${b}fill="${tc}">${preco}</text>`;
}

function trust(y: number): string {
  const it = (x: number, txt: string) =>
    `<circle cx="${x}" cy="${y - 4}" r="7" fill="none" stroke="#22E06B" stroke-width="1.5"/><text x="${x + 14}" y="${y}" font-family="Arial" font-size="13" fill="#938ea1">${txt}</text>`;
...
export function montarArteOferta(d: DadosArte, tall = false): string {
  ...
  const foto = d.fotoDataUri
    ...
    : `<rect x="344" y="${330 + topPad}" width="272" height="150" rx="12" fill="#1E1E20" stroke="#26262b"/><text x="480" y="${410 + topPad}" text-anchor="middle" font-family="Arial" font-size="13" fill="#5F5E5A">FOTO DO PRODUTO</text>`;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${V} ${hh}">`
    ...
    + `<text x="255" y="92" font-family="Arial" font-size="38" font-weight="700"><tspan fill="#FFFFFF">Promo</tspan><tspan fill="#22E06B">Detec</tspan></text>`
    + `<text x="340" y="126" text-anchor="middle" font-family="Arial" font-size="12" letter-spacing="5" fill="#22E06B" opacity="0.85">DETECTA · COMPARA · ECONOMIZA</text>`
    + `<text x="40" y="238" font-family="Arial" font-size="46" font-weight="700" fill="#FFFFFF">Achado</text>`
    + `<text x="40" y="286" font-family="Arial" font-size="46" font-weight="700" fill="#22E06B">do dia.</text>`
    + `<rect x="40" y="300" width="96" height="4" rx="2" fill="#22E06B"/>`
    + `<text x="40" y="340" font-family="Arial" font-size="19" fill="#FFFFFF">Preço real, verificado</text>`
    + `<text x="40" y="365" font-family="Arial" font-size="19" font-weight="700" fill="#22E06B">pelo histórico.</text>`
    ...
    + `<rect x="344" y="214" width="62" height="28" rx="8" fill="#22E06B"/><text x="375" y="233" text-anchor="middle" font-family="Arial" font-size="14" font-weight="700" fill="#0A0A0C">${esc(desconto)}</text>`
    ...
    + `<text x="600" y="239" text-anchor="middle" font-family="Arial" font-size="18" font-weight="700" fill="#cabeff">${score}</text>`
    + `<text x="600" y="276" text-anchor="middle" font-family="Arial" font-size="9" letter-spacing="1" fill="#938ea1">PROMOSCORE</text>`
    + `<text x="344" y="292" font-family="Arial" font-size="20" font-weight="700" fill="#FFFFFF">${nm[0]}</text>`
    + (nm[1] ? `<text x="344" y="316" font-family="Arial" font-size="20" font-weight="700" fill="#FFFFFF">${nm[1]}</text>` : "")
    + foto
    + `<text x="344" y="528" font-family="Arial" font-size="38" font-weight="700" fill="#22E06B">${esc(formatBRL(d.precoAtual))}</text>`
    + (d.precoOriginal && d.precoOriginal > d.precoAtual
      ? `<text x="344" y="554" font-family="Arial" font-size="17" fill="#938ea1">${esc(formatBRL(d.precoOriginal))}</text>`
        + `<line x1="344" y1="548" x2="${344 + formatBRL(d.precoOriginal).length * 9}" y2="548" stroke="#938ea1" stroke-width="1.5"/>`
      : "")
    + `<text x="40" y="608" font-family="Arial" font-size="13" fill="#938ea1">Histórico de preço deste produto</text>`
    ...
    + `<text x="96" y="758" font-family="Arial" font-size="16" font-weight="700" fill="#FFFFFF">Preço real, não vitrine.</text>`
    + `<text x="96" y="778" font-family="Arial" font-size="12" fill="#938ea1">${esc(d.lojaNome)} · histórico + PromoScore</text>`
    + `<rect x="436" y="744" width="184" height="38" rx="19" fill="#22E06B"/><text x="528" y="768" text-anchor="middle" font-family="Arial" font-size="13" font-weight="700" fill="#0A0A0C">→ ${link}</text>`
    + trust(820)
    + `</g></svg>`;
}
