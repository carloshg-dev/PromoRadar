...
const POR_PAGINA = 8;
const MAX_PRODUTOS_NO_CLIENTE = 64;
const INTERVALO_ROTACAO_MS = 5_000;
...
export interface OfertasMercadoLivreProps {
  /** Pool limitado e já filtrado para Mercado Livre pelo Server Component. */
  produtos: Produto[];
}

export function OfertasMercadoLivre({ produtos }: OfertasMercadoLivreProps) {
    ...
    <section className="pb-14">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-muted">Produtos coletados e atualizados, com saída registrada pelo PromoDetec.</p>
        </div>
      ...
      >
        {visiveis.map((produto) => {
          ...
          return (
            <a
              key={produto.id}
              href={`/r/${encodeURIComponent(produto.id)}?o=ofertas-mercadolivre`}
              target="_blank"
            ...
            >
              <div className="mb-3 flex items-center justify-between gap-2">
                <span className="inline-flex h-7 items-center rounded-md bg-[#FFE600] px-2 text-[10px] font-bold text-[#2D3277]">
                  <span className="sm:hidden">ML</span>
                  <span className="hidden sm:inline">Mercado Livre</span>
                </span>
                <span className="inline-flex h-7 shrink-0 items-center gap-1 rounded-lg bg-[#FFE600]/15 px-2 text-[10px] font-semibold text-[#FFE600]">
