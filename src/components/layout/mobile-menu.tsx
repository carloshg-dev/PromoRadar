...
export function MobileMenu() {
  ...
  // Garante que createPortal só roda no cliente (não durante SSR).
  ...
  return (
    <div className="md:hidden">
      ...
      {/*
        OVERLAY via portal — escapa do backdrop-filter do <header> que transformava
        o overlay fixed em "fixed dentro do header" (não cobria a página).
        z-[39]: acima do conteúdo (z-auto) mas abaixo do header sticky (z-40),
        então clicar em qualquer área fora do menu fecha. O header + nav ficam visíveis.
      */}
      {open && mounted && createPortal(
        <div
          aria-hidden
          onClick={fechar}
          className="fixed inset-0 z-[39] cursor-default bg-black/70"
        />,
        document.body
      )}
